...
use app\appResults\User;
use app\appForms\LoginForm;
...
class LoginCtrl{
	...
	public function validate() {
		...
		if (! getMessages()->isError ()) {
			...
			if ($this->form->login == "") {
				getMessages()->errTab ( 'Nie podano loginu' );
			}
			if ($this->form->pass == "") {
				getMessages()->errTab ( 'Nie podano hasła' );
			}
		}
		...
		if ( !getMessages()->isError() ) {
			...
			if ($this->form->login == "admin" && $this->form->pass == "admin") {
				...
				getMessages()->errTab('Niepoprawny login lub hasło');
			}
		}
	...
	public function generateView(){
		...
		getSmarty()->display('calcLogin.html');		
	}
}